...
if ($action === 'update_flight_number') {
    $stmt = $pdo->prepare('UPDATE flights SET flight_number=? WHERE id=? AND trip_id=?');
    $stmt->execute([trim((string)($_POST['flight_number'] ?? '')), (int)$_POST['flight_id'], $tripIdPost]);
    redirect_to_trip($tripIdPost);
}
...
<div class="page"><div class="page-wrapper">
    ...
    <div class="page-body"><div class="container-xl"><div class="row row-cards">
        ...
        <?php else: ?>
            ...
            <div class="card mb-3"><div class="card-header"><h3 class="card-title"><i class="ti ti-plane me-2"></i>Flights</h3></div><div class="table-responsive"><table class="table table-vcenter app-flights-table"><colgroup><col class="app-flight-date-col"><col class="app-flight-number-col"><col><col class="app-flight-time-col"><col class="app-flight-action-col"></colgroup><thead><tr><th>Date</th><th>Flight</th><th>Route</th><th>Time</th><th class="no-print"></th></tr></thead><tbody>
                <?php foreach ($flights as $f): ?><tr><td class="app-flight-date"><?= h($f['flight_date']) ?></td><td class="app-flight-info"><strong><?= h($f['airline']) ?></strong><form method="post" class="d-flex gap-1 mt-1 no-print app-flight-number-form"><input type="hidden" name="action" value="update_flight_number"><input type="hidden" name="trip_id" value="<?= $tripId ?>"><input type="hidden" name="flight_id" value="<?= (int)$f['id'] ?>"><input name="flight_number" class="form-control form-control-sm flight-number-input" value="<?= h($f['flight_number']) ?>" aria-label="Flight number"><button class="btn btn-sm btn-outline-primary" aria-label="Save flight number"><i class="ti ti-device-floppy"></i></button></form><span class="text-secondary d-print-inline d-none"><?= h($f['flight_number']) ?></span></td><td class="app-flight-route"><?= h($f['departure_airport']) ?> → <?= h($f['arrival_airport']) ?></td><td class="app-flight-time"><?= h($f['departure_time']) ?> - <?= h($f['arrival_time']) ?></td><td class="no-print app-flight-actions"><form method="post"><input type="hidden" name="action" value="delete_flight"><input type="hidden" name="trip_id" value="<?= $tripId ?>"><input type="hidden" name="flight_id" value="<?= (int)$f['id'] ?>"><button class="btn btn-sm btn-outline-danger"><i class="ti ti-trash"></i></button></form></td></tr><?php endforeach; ?>
